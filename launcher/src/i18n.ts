...
const en = {
  ...
  chatClean: "Chat Clean",
  chatCleanBody: "Improve retained-chat performance by keeping only the latest 3 user messages and their 3 responses visible. Enabled by default in Automatic mode.",
...
const zh: Record<keyof typeof en, string> = {
  ...
  chatClean: "聊天清理",
  chatCleanBody: "仅保留最近 3 条用户消息及其 3 条回复，以提升保留聊天的性能。自动模式下默认启用。",
...
const ja: Record<keyof typeof en, string> = {
  ...
  chatClean: "Chat Clean",
  chatCleanBody: "保持されたチャットの性能を高めるため、最新のユーザーメッセージ 3 件と応答 3 件だけを表示します。自動モードでは初期設定で有効です。",
...
const ko: Record<keyof typeof en, string> = {
  ...
  chatClean: "Chat Clean",
  chatCleanBody: "유지된 채팅에서 최신 사용자 메시지 3개와 해당 응답 3개만 표시하여 성능을 개선합니다. 자동 모드에서 기본적으로 활성화됩니다.",
...
const zhTW: Record<keyof typeof en, string> = {
  ...
  chatClean: "Chat Clean",
  chatCleanBody: "只保留最新 3 則使用者訊息及其 3 則回覆顯示，可改善保留聊天的效能。自動模式預設啟用。",

...
test("Chat Clean preserves the new data-turn-key logical shell", () => {
  const clean = (browserWorkerExports as Record<string, unknown>).cleanChatGptConversationDocument;
  expect(typeof clean).toBe("function");
  if (typeof clean !== "function") return;

  const attributes = new Map<string, string>([["data-turn-key", "turn-1"]]);
  const group = {
    role: "user",
    hasAttribute: (name: string) => attributes.has(name),
    closest: () => null,
    remove: () => { throw new Error("stable data-turn-key shell must not be removed"); },
    replaceChildren: () => attributes.set("children", "removed"),
    removeAttribute: (name: string) => attributes.delete(name),
    setAttribute: (name: string, value: string) => attributes.set(name, value),
  };
  const retained = Array.from({ length: 6 }, (_unused, index) => ({
    role: index < 3 ? "user" : "assistant",
    hasAttribute: () => false,
    closest: () => ({}),
    remove: () => { throw new Error("retained turn must not be removed"); },
  }));
  const turns = [group, ...retained];
  const root = {
    querySelectorAll: (selector: string) => selector === "user turns"
      ? turns.filter(turn => turn.role === "user")
      : selector === "assistant turns"
  ...
  expect((clean as Function)({
    turnSelector: "all turns",
    userTurnSelector: "user turns",
    assistantTurnSelector: "assistant turns",
  }, root)).toEqual({ found: 7, pruned: 1, retained: 6 });
  expect(attributes.get("data-turn-key")).toBe("turn-1");
  expect(attributes.get("children")).toBe("removed");
  expect(attributes.get("aria-hidden")).toBe("true");
  expect(attributes.get("data-chat-cleaned")).toBe("true");
});
...
test("browser turns queue beyond the five-tab limit instead of disconnecting", async () => {
  ...
  let resolveFirstFiveStarted!: () => void;
  const firstFiveStarted = new Promise<void>(resolve => { resolveFirstFiveStarted = resolve; });
  ...
  const worker = Object.assign(Object.create(ChatGptBrowserWorker.prototype), {
    ...
    runExclusive: (turn: { traceId: string }) => new Promise<string>(resolve => {
      ...
      if (releases.size === 5 && !releases.has("trace_6")) resolveFirstFiveStarted();
  ...
  await Promise.race([
    firstFiveStarted,
    Bun.sleep(250).then(() => { throw new Error("initial five browser turns did not start"); }),
  ]);
...
test("launcher DOM-health failures are classified for same-turn page recovery", () => {
  const error = new ChatGptDomHealthObservationError("ChatGPT response DOM disappeared");
  expect(error.name).toBe("ChatGptDomHealthObservationError");

  const worker = readFileSync("src/adapters/chatgpt-web/browser-worker.ts", "utf8");
  expect(worker).toContain("error instanceof ChatGptDomHealthObservationError");
  expect(worker).toContain("domHealthTracker = new ChatGptTurnDomHealthTracker()");
  expect(worker).toContain('"response-dom-health-rebound"');
});
